# Deploy manual — Unio Jurídico (PC → HostGator)
...
| 2 | PC | `deploy-uniojuridico-manual.ps1` envia código via SSH |
| 3 | Servidor | `deploy-server.sh` — migrate, cache, sync `public_html` |

**URL:** https://uniojuridico.uniowork.com.br  
**App:** `/home2/joabef36/unio-uniojuridico`  
...
copy config\deploy-uniojuridico.local.env.example config\deploy-uniojuridico.local.env
```

Edite `config/deploy-uniojuridico.local.env` e ajuste `DEPLOY_KEY_FILE`.
...
Na raiz do projeto, branch `uniojuridico`:
...
4. `scp` → servidor `/tmp/deploy-uniojuridico.tar.gz`
5. `ci-remote-extract.sh` → extrai + `deploy-server.sh`
6. Smoke em `https://uniojuridico.uniowork.com.br/login`

### Só reenviar (sem rebuild local)

```powershell
powershell -ExecutionPolicy Bypass -File scripts\deploy-uniojuridico-manual.ps1 -SkipBuild
```
...
Detalhes: [UNIOJURIDICO_DEPLOY_REPAIR.md](UNIOJURIDICO_DEPLOY_REPAIR.md)
...
```
/home2/joabef36/unio-uniojuridico/var/log/deploy-report.txt
```
...
**Recomendação atual:** `git push` para versionar + `deploy-uniojuridico-manual.ps1` para publicar.

**Acessos em produção:** [UNIOJURIDICO_ACESSOS.md](UNIOJURIDICO_ACESSOS.md)
