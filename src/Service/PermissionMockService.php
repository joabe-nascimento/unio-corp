...
/**
 * Escopos de permissão por hub/produto — mock até persistência real.
...
class PermissionMockService
{
    ...
    private const ASSIGNABLE_PROFILES = [
    ...
    /** @var array<string, array<string, string>> scope => member_id => product_id => perfil_id */
    ...
    public function getScopeData(string $scope): ?array
    {
        ...
        $defaults = self::DEFAULT_GRANTS[$scope] ?? [];

        $rows = [];
        foreach ($members as $member) {
            $grants = $defaults[$member['id']] ?? [];
            $allGrants = $this->getAllGrantsForMember($member['id']);
            $rows[] = [
                'member' => $member,
                'grants' => $grants,
     ...
     */
    public function getAllGrantsForMember(string $memberId): array
    {
        $grants = [];
    ...
    /**
     * @return list<array{id: string, nome: string, email: string, initials: string, equipe: string, cargo: string, perfil_global: string, perfil_label: string, perfil_class: string}>
     */
    private function getMembers(): array
    {
        return [
            $this->member('gestor', 'Gestor Oliveira', 'gestor@huplex.dev', 'GESTOR', 'PMO', 'Gestor de Operações', 3),
            $this->member('gestor-eq', 'Gestor Costa', 'gestor.eq@huplex.dev', 'GESTOR_EQUIPE', 'Squad Backend', 'Gestor de Equipe', 2),
            $this->member('supervisor', 'Supervisor Geral', 'supervisor@huplex.dev', 'SUPERVISOR', '—', 'Supervisor Geral', 9),
            $this->member('sup-eq', 'Supervisor Equipe', 'sup.eq@huplex.dev', 'SUPERVISOR_EQUIPE', 'Obras e Projetos', 'Supervisor de Campo', 4),
            $this->member('membro', 'Membro Santos', 'membro@huplex.dev', 'MEMBRO', 'Design & Marca', 'Analista', 8),
        ];
    }

    /**
     * @return array{id: string, nome: string, email: string, initials: string, equipe: string, cargo: string, perfil_global: string, perfil_label: string, perfil_class: string, ficha_id: int|null}
     */
    private function member(string $id, string $nome, string $email, string $perfil, string $equipe, string $cargo, ?int $fichaId = null): array
    {
        ...
        return [
            ...
            'perfil_label' => $labels[$perfil] ?? $perfil,
            'perfil_class' => $classes[$perfil] ?? 'default',
            'ficha_id' => $fichaId,
        ];
    }
}
