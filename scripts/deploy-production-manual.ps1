# Deploy manual Producao (uniowork.com.br) — PC (Windows) -> HostGator via SSH.
# Espelha o pipeline do GitHub Actions sem depender do Actions.
#
# Uso (na raiz do repo, branch production):
#   powershell -ExecutionPolicy Bypass -File scripts\deploy-production-manual.ps1
#
# Pre-requisitos:
#   - OpenSSH (ssh/scp) no PATH
#   - tar (Windows 10+)
#   - composer, npm, php
#   - config/deploy-production.local.env com DEPLOY_KEY_FILE

param(
    [switch]$SkipBuild,
    [string]$ArchiveName = "deploy-production.tar.gz"
)
...
Write-Host "== Deploy manual Producao uniowork.com.br (PC -> HostGator) ==" -ForegroundColor Cyan

$cfg = @{}
Load-EnvFile "$root\config\deploy-hostgator.defaults.env" $cfg
$localEnv = "$root\config\deploy-production.local.env"
if (-not (Test-Path $localEnv)) {
    Write-Host ""
    Write-Host "ERRO: crie config/deploy-production.local.env" -ForegroundColor Red
    Write-Host "      Copie de config/deploy-production.local.env.example" -ForegroundColor Red
...
if (-not $deployPath) { $deployPath = "/home2/joabef36/unio" }
$publicHtml = $cfg["DEPLOY_PUBLIC_HTML"]
if (-not $publicHtml) { $publicHtml = "/home2/joabef36/public_html" }
...
$reportLocal = "$root\deploy-reports\deploy-report-production.txt"
...
if (-not $defaultUri) { $defaultUri = "https://uniowork.com.br" }
Write-Host ""
