...
#   PHP_BIN              php (default)
#   APP_ENV              dev (default)
#   VALIDATE_FRESH_DB    1 = drop/create/schema/seed como no CI
#   QUICK                1 = pula PHPStan, PHPUnit e npm
#   SKIP_PHPSTAN         1 = pula PHPStan
#   SKIP_TESTS           1 = pula PHPUnit
#   SKIP_ASSETS          1 = pula npm ci + vendor:sync
#   SKIP_DB              1 = pula tudo que precisa de banco
#   GIT_BRANCH           branch para regra do PHPStan (auto-detect)
...
run_step() {
  step=$((step + 1))
  echo ""
  echo -e "${BLD}==> [$step] $1${RST}"
}

fail() {
  echo ""
  echo -e "${RED}VALIDAÇÃO FALHOU${RST}: $1" >&2
  exit 1
}
...
if [[ "$QUICK" != "1" && "$SKIP_PHPSTAN" != "1" ]]; then
  ...
  echo ""
  echo "  ⚠ PHPStan ignorado (branch production ou SKIP_PHPSTAN=1)."
fi
