/**
 * Unio Offcanvas — painel lateral reutilizável
 *
...
(function () {
    ...
    function scan() {
        document.querySelectorAll('[data-unio-offcanvas]').forEach(register);
    }

    function resolveOpenId(el) {
        var trigger = el.closest('[data-unio-offcanvas-open]');
        if (trigger) {
            return trigger.getAttribute('data-unio-offcanvas-open');
        }
        var node = el.closest('[data-unio-offcanvas-open], button, a');
        ...
        for (var i = 0; i < node.attributes.length; i++) {
            ...
            if (match && name !== 'data-unio-offcanvas-open') {
                return match[1];
            }
        }
        return null;
    ...
    function resolveCloseRoot(el) {
        var scoped = el.closest('[data-unio-offcanvas-close]');
        if (scoped) {
            return scoped.closest('[data-unio-offcanvas]');
        }
        ...
        for (var j = 0; j < node.attributes.length; j++) {
            ...
            if (m && n !== 'data-unio-offcanvas-close') {
                return document.querySelector('[data-unio-offcanvas="' + m[1] + '"]');
            }
        }
        return null;
    }
    ...
    function open(id) {
        var root = panels[id] || document.querySelector('[data-unio-offcanvas="' + id + '"]');
        if (!root) {
            return;
        }
        register(root);
    ...
    function close(id, removeBodyClass) {
        ...
        if (removeBodyClass !== false) {
            var anyOpen = document.querySelector('[data-unio-offcanvas].is-open');
            if (!anyOpen) {
    ...
    function onClick(e) {
        ...
        if (closeRoot) {
            e.preventDefault();
            var cid = closeRoot.getAttribute('data-unio-offcanvas');
            if (cid) {
                close(cid);
            }
        }
    ...
    function onKeydown(e) {
        ...
        var opened = document.querySelector('[data-unio-offcanvas].is-open');
        if (!opened) {
            return;
        }
        var cid = opened.getAttribute('data-unio-offcanvas');
        if (cid) {
            close(cid);
        }
    }
